...
df = df[df['some column header'].notna()] # Pulls the value (number) from the column if applicable.
df.to_csv('csv_to.csv', index=False)

read_csv = pd.read_csv('csv_to.csv')
# dtype_before = type(test['hold number']) # Getting the type.
asset_list = read_csv['asset'].tolist()
hold_list = read_csv['hold number'].tolist()
...
mail.Subject = 'A random subject title'
mail.Body = ''
